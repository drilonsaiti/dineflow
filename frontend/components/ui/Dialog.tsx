...
>(({ className = '', children, ...props }, ref) => (
    <DialogPrimitive.Portal>
        <DialogPrimitive.Overlay
            className="fixed inset-0 z-40 bg-black/50 data-[state=open]:animate-in data-[state=open]:fade-in"
        />
...
export const DialogTitle = ({
    ...
    <DialogPrimitive.Title
        className={`text-lg font-semibold text-ink dark:text-white ${className}`}
        {...props}
    />
);
...
export const DialogDescription = ({
    ...
    <DialogPrimitive.Description
        className={`mt-2 text-sm text-muted ${className}`}
        {...props}
    />
);
