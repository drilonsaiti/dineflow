...
export function ItemForm({ venueId, categoryId, existing, venueTags, onDone }: Props) {
    ...
    function addOption(groupIndex: number) {
        updateGroup(groupIndex, {
            options: [...groups[groupIndex].options, { name: '', priceDeltaCents: 0, displayOrder: groups[groupIndex].options.length }],
        });
    }
        ...
        <form onSubmit={submit} className="mt-3 space-y-4 rounded-lg bg-surface-card p-4 dark:bg-surface-dark-elevated">
            <div className="flex gap-3">
                ...
                <div className="w-28">
                    ...
                    <input className="input mt-1" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="12.50" inputMode="decimal" />
                </div>
            </div>
            ...
            <div className="flex gap-3">
                <div className="flex-1">
                    ...
                    <input type="time" className="input mt-1" value={availableFrom} onChange={(e) => setAvailableFrom(e.target.value)} />
                </div>
                <div className="flex-1">
                    <label className="block text-xs font-medium text-muted">Available until</label>
                    <input type="time" className="input mt-1" value={availableTo} onChange={(e) => setAvailableTo(e.target.value)} />
                </div>
            </div>
            ...
            <div>
                ...
                <div className="mt-1 flex items-center gap-3">
                    ...
                    <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="btn-secondary px-3 py-1.5 text-sm">
            ...
            <div className="flex gap-3">
                <div className="flex-1">
                    ...
                    <input type="number" min={0} className="input mt-1" placeholder="Leave blank = unlimited" value={stockCount} onChange={(e) => setStockCount(e.target.value)} />
                </div>
                {stockCount !== '' && (
                    <div className="w-32">
                        <label className="block text-xs font-medium text-muted">Low stock warning at</label>
                        <input type="number" min={0} className="input mt-1" placeholder="5" value={lowStockThreshold} onChange={(e) => setLowStockThreshold(e.target.value)} />
                    </div>
                )}
            </div>
            ...
            <div className="space-y-3">
                <div className="flex items-center justify-between">
                    ...
                    <button type="button" className="text-sm font-medium text-ink hover:underline dark:text-white" onClick={addGroup}>
                        + Add group
                    </button>
                </div>
                ...
                {groups.map((group, gi) => (
                    <div key={gi} className="card space-y-2 p-3">
                        <div className="flex items-center gap-2">
                            <input className="input flex-1 py-1 text-sm" placeholder="Group name, e.g. Size" value={group.name} onChange={(e) => updateGroup(gi, { name: e.target.value })} />
                            ...
                            <input type="number" className="input w-14 py-1 text-xs" value={group.minSelect} onChange={(e) => updateGroup(gi, { minSelect: Number(e.target.value) })} title="Min select" />
                            <input type="number" className="input w-14 py-1 text-xs" value={group.maxSelect} onChange={(e) => updateGroup(gi, { maxSelect: Number(e.target.value) })} title="Max select" />
                            <button type="button" className="text-xs font-medium text-error" onClick={() => removeGroup(gi)}>Remove</button>
                        </div>

                        <div className="space-y-1 pl-2">
                            {group.options.map((option, oi) => (
                                <div key={oi} className="flex items-center gap-2">
                                    <input className="input flex-1 py-1 text-sm" placeholder="Option name, e.g. Large" value={option.name} onChange={(e) => updateOption(gi, oi, { name: e.target.value })} />
                                    <input
                                    ...
                                    />
                                    <button type="button" className="text-xs font-medium text-error" onClick={() => removeOption(gi, oi)}>Remove</button>
                                </div>
                            ))}
                            <button type="button" className="text-xs font-medium text-ink hover:underline dark:text-white" onClick={() => addOption(gi)}>
                                + Add option
                            </button>
                        </div>
                    </div>
